<template>
  <div class="hm-home">
    <header>
      ...
      <div class="center">
        <span class="iconfont iconsearch" @click="$router.push('/search')">搜索新闻</span>
      </div>
      <div class="right">
    ...
    <van-tabs v-model="active" sticky animated swipeable>
      <van-icon @click="$router.push('/tab-manage')" name="comment-o" color="#1989fa" size="40" />
      <van-tab :title="item.name" v-for="item in tabList" :key="item.id">
        ...
        <van-pull-refresh v-model="refreshing" @refresh="onrefresh">
          ...
          <van-list
            ...
            :offset="50"
          >
            <hm-post :article="post" v-for="post in articleList" :key="post.id"></hm-post>
          </van-list>
        </van-pull-refresh>
      </van-tab>
    </van-tabs>
